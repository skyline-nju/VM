...
void Bird_wo_pos::set_para(double LX, double LY, double V0) {
  Lx = LX;
  Ly = LY;
  v0 = V0;
}
...
void Bird_wo_pos::scalar_noise(double eta, double disorder, Ran & myran) {
  double tmp = std::sqrt(vx_next * vx_next + vy_next * vy_next);
  double c1 = vx / tmp;
  double s1 = vy / tmp;
  double noise;
  if (eta > 0) {
    noise = myran.doub() * eta * 2 * PI + disorder;
  } else {
    noise = disorder;
  }
  double c2 = cos(noise);
